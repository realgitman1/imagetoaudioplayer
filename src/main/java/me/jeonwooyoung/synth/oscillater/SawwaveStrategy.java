...
public class SawwaveStrategy implements Osc1 {

    private final double frequency;
    private Synthesizer synth; // 멤버 변수로 선언하고 private으로 캡슐화
    private SawtoothOscillator sawtoothOscillator; // 사인파 오실레이터
    private LineOut lineOut; // 오디오 출력

    public SawwaveStrategy(double frequency) {
        ...
        sawtoothOscillator.output.connect(0, lineOut.input, 0);
        sawtoothOscillator.output.connect(0, lineOut.input, 1); // 스테레오로 들리게 양쪽 채널에 연결
    }
    ...
    public void startAudio() {
        ...
        sawtoothOscillator.frequency.set(((frequency * 78.4156) + 40)/4); //
        sawtoothOscillator.amplitude.set(0.8);   //
