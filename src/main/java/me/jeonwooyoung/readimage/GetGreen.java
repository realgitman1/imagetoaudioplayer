...
public class GetGreen implements GetAverage {
    ...
    public GetGreen(ReadImages readImages){
    ...
    public double GetAverageLevel() {
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++){
                if (pixels[j][i] != null) {
                    currentSum += pixels[j][i].getGreen(); // getBrightness() 호출 (Pixel 클래스에 이 메서드가 있어야 함)
                } else {
